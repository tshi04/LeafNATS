...
class modelABS(modelSeq2SeqBase):
    ...
    def build_batch(self, batch_id):
        ...
        self.batch_data['src_mask_pad'] = output['src_mask_pad'].to(
            self.args.device)
        ...
        if self.args.task == 'train' or self.args.task == 'validate':
            ...
            self.batch_data['trg_output'] = output['trg_output_var'].to(
                self.args.device)
            self.batch_data['trg_seq_len'] = self.batch_data['trg_input'].size(
                1)
        else:
    ...
    def build_decoder_one_step(self, k=0):
        ...
        if k == 0:
            ...
            # (seqL+1)*batch**hidden
            past_dech = torch.cat((past_dech, hidden[0]), 0)
            past_dech = past_dech.view(
                dec_idx +
                1, self.batch_data['batch_size'], self.args.trg_hidden_dim
            )  # (seqL+1)*batch*hidden
    ...
    def build_pipelines(self):
        ...
        pad_mask = torch.ones(
            self.batch_data['vocab_size']).to(self.args.device)
