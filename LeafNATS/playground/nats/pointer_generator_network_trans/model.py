...
from LeafNATS.playground.summarization.pointer_generator_network.model import \
...
class modelNatsTransfer(modelPointerGenerator):
    ...
    def build_pipelines(self):
        ...
        '''
        with torch.no_grad():
            src_emb = self.base_models['embedding_base'].get_embedding(
        ...
        with torch.no_grad():
            trg_emb = self.base_models['embedding_base'].get_embedding(
        ...
        if self.args.share_emb_weight:
            ...
            with torch.no_grad():
                logits_ = self.base_models['embedding_base'].get_decode2vocab(
                    decoder_proj)
        else:
