...
from LeafNATS.modules.nats_attention_encoder import AttentionEncoder
from LeafNATS.modules.nats_attention_decoder import AttentionDecoder
...
class PointerGeneratorDecoder(torch.nn.Module):
    ...
    def forward(
        self, 
        idx, # index of current decoding step (used in testing, set 0 for training)
        ...
        for k in range(input_.size(0)):
            ...
            if self.attn_decoder:
                ...
                if k + idx == 0:
                    ...
                    past_dehy = torch.cat((past_dehy, hhh_), 0) # (seqL+1)*batch**hidden
                ...
                h_attn = self.attn_out(torch.cat((c_encoder, c_decoder, hhh_), 1))
            else:
