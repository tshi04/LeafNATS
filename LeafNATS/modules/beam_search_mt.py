...
def fast_beam_search(
    args, models, batch_data,
    src_text_rep, src_text_rep_ex, batch_size, task_key
):
    ...
    if args.task_key == 'summary':
        max_len = args.sum_seq_lens
    if args.task_key == 'title':
    ...
    for j in range(max_len):
        if args.oov_explicit:
            last_wd[last_wd>=len(batch_data['vocab2id'])] = batch_data['vocab2id']['<unk>']
        ...
        if args.pointer_net:
            if args.oov_explicit and ex_vocab_size > 0:
                ...
                logitsex = logitsex.repeat(batch_size*beam_size, 1, ex_vocab_size)
                if ex_vocab_size > 0:
                ...
                pt_idx = pt_idx.repeat(batch_size*beam_size, src_seq_len, vocab_size)
