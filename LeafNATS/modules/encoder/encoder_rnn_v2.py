...
class EncoderRNN(torch.nn.Module):
    ...
    def forward(self, input_):
        ...
        h0_encoder = Variable(torch.zeros(n_dk*self.nLayers, batch_size, self.hidden_size)).to(self.device)
        if self.rnn_network == 'lstm':
            c0_encoder = Variable(torch.zeros(n_dk*self.nLayers, batch_size, self.hidden_size)).to(self.device)
            # encoding
