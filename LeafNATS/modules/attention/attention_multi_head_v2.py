...
class MultiHeadedAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        self.n_dk = input_size // n_heads
        self.n_heads = n_heads

        self.proj_query = torch.nn.Linear(input_size, input_size)
        self.proj_key = torch.nn.Linear(input_size, input_size)
        self.proj_value = torch.nn.Linear(input_size, input_size)
        self.dropout = torch.nn.Dropout(drop_rate)
        
        self.proj_output = torch.nn.Linear(input_size, hidden_size)
    ...
    def forward(self, input_, mask=None):
        ...
        query = query.view(batch_size, -1, self.n_heads, self.n_dk).transpose(1, 2)
