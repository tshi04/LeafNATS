...
from nli.model_base_pre_emb import modelNLIBasePreEmb
...
class modelNLI(modelNLIBasePreEmb):
    ...
    def build_pipe(self):
        ...
        prem_maxp = F.max_pool1d(prem_enc.transpose(
            1, 2), prem_enc.size(1)).squeeze(2)
        hypo_maxp = F.max_pool1d(hypo_enc.transpose(
            1, 2), hypo_enc.size(1)).squeeze(2)

        prem_avgp = F.avg_pool1d(prem_enc.transpose(
            1, 2), prem_enc.size(1)).squeeze(2)
        hypo_avgp = F.avg_pool1d(hypo_enc.transpose(
            1, 2), hypo_enc.size(1)).squeeze(2)

        enc_cat = torch.cat([prem_maxp, hypo_maxp, prem_avgp, hypo_avgp], 1)

        fc = torch.relu(self.train_models['drop'](
            self.train_models['ff1'](enc_cat)))
        fc = torch.relu(self.train_models['drop'](
            self.train_models['ff2'](fc)))
